...
| 2026-07-12 | 51-spring-integration | `EnableIntegration` / `IntegrationComponentScan` no compilan | En Spring Integration 7.1 `@IntegrationComponentScan` se movió a `org.springframework.integration.annotation.*` (antes en `org.springframework.integration.config.*`) | Actualizar el import a `org.springframework.integration.annotation.IntegrationComponentScan`. |
| 2026-07-12 | 59-reactive | `@AutoConfigureWebTestClient` no compila; H2 R2DBC "table not found (candidates ITEMS)" | En Boot 4.1.0 se eliminó `@AutoConfigureWebTestClient`. Además H2 por defecto usa identificadores en mayúsculas mientras Spring Data R2DBC busca `@Table("items")` en minúsculas | 1) Construir `WebTestClient` manualmente con `bindToServer().baseUrl("http://localhost:"+port).build()` con `@LocalServerPort`. 2) Añadir `DATABASE_TO_LOWER=TRUE;CASE_INSENSITIVE_IDENTIFIERS=TRUE` a la URL R2DBC de H2. |
