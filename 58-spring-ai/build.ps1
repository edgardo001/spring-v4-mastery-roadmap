# Build script (PowerShell) para el modulo 58-spring-ai.
...
try {
    & $Mvn -q clean package
    Write-Host ""
    Write-Host "Artefacto: target/spring-ai-1.0.0.jar"
    Write-Host "Ejecutar:  java -jar target/spring-ai-1.0.0.jar"
} finally {
    Pop-Location
}
