# Build script (PowerShell) para el módulo 18-jpa-avanzado.
...
try {
    & $Mvn -q clean package
    Write-Host ""
    Write-Host "Artefacto: target/jpa-avanzado-1.0.0.jar"
    Write-Host "Ejecutar:  java -jar target/jpa-avanzado-1.0.0.jar"
} finally {
    Pop-Location
}
