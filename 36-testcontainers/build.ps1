# Build script (PowerShell) para el módulo 36-testcontainers.
...
try {
    & $Mvn -q clean package
    Write-Host ""
    Write-Host "Artefacto: target/testcontainers-1.0.0.jar"
    Write-Host "Ejecutar:  java -jar target/testcontainers-1.0.0.jar"
} finally {
    Pop-Location
}
